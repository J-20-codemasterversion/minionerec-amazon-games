...
def train(
    ...
    # PATCH (J-20-codemasterversion): added "Games" mapping; original dict crashed with KeyError.
    category_dict = {"Industrial_and_Scientific": "industrial and scientific items", "Office_Products": "office products", "Games": "video games", "Toys_and_Games": "toys and games", "Sports": "sports and outdoors", "Books": "books"}
    print(category)
    ...
    if sid_index_path and os.path.exists(sid_index_path):
        ...
        if new_tokens:
            # ---------- PATCH (J-20-codemasterversion): original_vocab_size NameError ----------
            # The original code only defined `original_vocab_size` inside the
            # `train_from_scratch=True` branch, but the freeze_LLM path below
            # references it unconditionally. With (train_from_scratch=False,
            # freeze_LLM=True) — the only configuration that fits a single V100 —
            # this raises NameError. Capture the size *before* extending the vocab.
